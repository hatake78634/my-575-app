...
import { AppShell, EmptyState, ErrorState, LoadingState, Surface } from '../../components/ui'
...
export default function LineagePage() {
  ...
  const renderBranch = (personId: string, direction: 'mentor' | 'apprentice', depth: number, visited: Set<string>): React.ReactNode => {
    ...
    return related.length > 0 && <ul style={{ display: 'grid', gap: 8 }}>{related.map((relatedId) => <li key={relatedId}>
      <Link href={`/user/${relatedId}`} className="ui-link">{people[relatedId]?.username || '名無しの歌人'}</Link>
      {renderBranch(relatedId, direction, depth + 1, nextVisited)}
    </li>)}</ul>
  }

  return <AppShell>
    <h1 style={{ color: 'var(--primary)' }}>一門図</h1>
    {loadedId !== id ? <LoadingState /> : errorText ? <ErrorState>{errorText}</ErrorState> : <>
      <Surface style={{ padding: 18 }}><h2>師匠の系譜</h2>{renderBranch(id, 'mentor', 0, new Set()) || <EmptyState>登録された師匠はいません。</EmptyState>}</Surface>
      ...
      <Surface style={{ padding: 18 }}><h2>弟子の系譜</h2>{renderBranch(id, 'apprentice', 0, new Set()) || <EmptyState>登録された弟子はいません。</EmptyState>}</Surface>
    </>}
  </AppShell>
}
